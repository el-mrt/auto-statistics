...
missings_server <- function(id, user_data, user_target_col){
  moduleServer(id, function(input, output, session){
    ...
    output$na_hist_plot <- renderPlot({
      ...
      if(input$na_hist_col2 == "None"){
        cur_plot <- ggplot(na_hist_data, aes(x = get(rV$target_column), fill = isna)) +
          geom_histogram(binwidth = input$na_hist_bins) +
          scale_fill_manual(values = c("#377EB8", "#BD3631")) +
          labs(x = rV$target_column) +
          theme_minimal()
      }else{
        cur_plot <- ggplot(na_hist_data, aes(x = rV$target_column, y = get(col_name2), color = isna)) +
          geom_jitter() +
          scale_color_manual(values = c("#377EB8", "#BD3631")) +
          labs(x = rV$target_column, y = col_name2) +
          theme_minimal()
      }
      cur_plot
    })
